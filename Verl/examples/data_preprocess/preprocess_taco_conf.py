...
"""
Preprocess the TACO dataset to verl parquet format with confidence prompting.
...
SYSTEM_PROMPT = (
    "You need to provide the answer as well as its confidence level to follow-up questions. "
    "The confidence level is a number between 0 and 1 (inclusive) enclosed within "
    "<confidence> </confidence> tags. The final format that must be followed is: "
    "<confidence> confidence level here (number between 0 and 1) </confidence> answer here. "
    "Ensure to output the confidence score first before answering."
)
...
def _build_prompt(question, starter_code, test_type, tests, fn_name):
    ...
    if test_type == "assert":
        ...
        if starter_code and starter_code.strip():
            ...
            prompt += "Return the final solution inside a Python code block.\n" "```python\n# YOUR CODE HERE\n```\n"
    else:
    ...
    def process_fn(example, idx):
        ...
        return {
            ...
            "prompt": [
                ...
                {"role": "user", "content": _build_prompt(question, starter_code, test_type, tests, fn_name)},
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_save_dir", default="~/data/taco_conf", help="Directory to save processed parquet files."
    )
    parser.add_argument(
        "--remove_columns",
        action="store_true",
